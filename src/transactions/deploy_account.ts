...
export async function generalDeployAccount(tx: starknet.TransactionWithHash, syncingProvider: starknet.RpcProvider) {
  ...
  switch (tx_version) {
    ...
    default: {
      throw new Error(`Unsupported Deploy Account transaction version: ${tx_version}`);
    }
  }
}

async function deployAccountV1(tx: starknet.TransactionWithHash, syncingProvider: starknet.RpcProvider) {
...
async function deployAccountV3(tx: starknet.TransactionWithHash, syncingProvider: starknet.RpcProvider) {
